...
export function MonsterDetailsPanel({ monsterId }: RealmDetailsPanelProps) {
  const router = useRouter();

  const { data: realmData, loading } = useGetRealmQuery({
    variables: { id: monsterId },
    pollInterval: 5000,
  });
  ...
  const toggleFavourite = () => {
    ...
    const isFavourite = favs.indexOf(monsterId) > -1;

    if (isFavourite) {
      storage<number[]>(RealmFavoriteLocalStorageKey, []).set(
        favs.filter((f) => f !== monsterId)
      );
      toast('Removed from Favorites');
    } else {
      storage<number[]>(RealmFavoriteLocalStorageKey, []).set(
        favs.concat(monsterId)
      );
